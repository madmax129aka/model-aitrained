...
/ exifread analysis with no network calls. ALL inference and analysis
happens server-side in this backend -- there is no browser-based
TensorFlow.js inference anywhere in this app.
"""
import uuid
...
def run_full_analysis(image: Image.Image, raw_bytes: bytes) -> dict:
    ...
    """
    # --- Signal A: our custom CNN (128x128 face-trained model) ------------------
    ...
    cnn_explanation = (
        f"Our custom-trained CNN (trained from scratch on the 140k Real and Fake "
        f"Faces dataset) estimates a {cnn_score:.1f}% probability this image is "
        f"AI-generated. The model's raw output favors the "
        f"'{cnn_result['predicted_label']}' class. This model was trained on faces, "
        f"so it is most reliable on photos containing a clear human face."
    )
